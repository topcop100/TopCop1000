...
public class MediaHubActivity extends Activity {
    ...
    final class HubView extends View{
        ...
        void openStream(){ if(playlistUrls.isEmpty())return; String url=playlistUrls.get(streamFocus).trim(); if("PORTALE".equals(mode)&&!stalkerToken.isEmpty()&&!(url.startsWith("http://")||url.startsWith("https://"))){resolveStalkerLink(url);return;} playResolvedStream(url); }
