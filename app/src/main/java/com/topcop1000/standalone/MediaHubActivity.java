...
public class MediaHubActivity extends Activity {
    ...
    final class HubView extends View{
        ...
        void deleteLiveLine(){
            ...
            final String[] a=liveLines.toArray(new String[0]);new AlertDialog.Builder(MediaHubActivity.this).setTitle("LINE LÖSCHEN").setItems(a,(d,which)->{String old=a[which];getSharedPreferences("topcop_live_lines",MODE_PRIVATE).edit().remove("fav_"+old.hashCode()).apply();int sep=old.indexOf('|');String name=(sep>=0?old.substring(0,sep):"LIVE LINE").trim(),url=(sep>=0?old.substring(sep+1):old).trim();FavoriteStore.remove(MediaHubActivity.this,"livetv",name+" | "+url);liveLines.remove(which);saveLiveLines();Toast.makeText(MediaHubActivity.this,"Line gelöscht",Toast.LENGTH_SHORT).show();}).setNegativeButton("Abbrechen",null).show();
        }
