...
public class FavoritesActivity extends Activity {
 ...
 final class FavView extends View{
  ...
  void openFavorite(String value){int sep=value.indexOf(" | ");if(sep<0){Toast.makeText(FavoritesActivity.this,"Favorit enthält keine Quelle",Toast.LENGTH_SHORT).show();return;}String url=value.substring(sep+3).trim();if(category==3&&isStalkerCommand(url)){Toast.makeText(FavoritesActivity.this,"Stalker-Favorit wird über PORTALE geöffnet",Toast.LENGTH_SHORT).show();android.content.Intent hub=new android.content.Intent(FavoritesActivity.this,MediaHubActivity.class);hub.putExtra(MediaHubActivity.EXTRA_MODE,"PORTALE");hub.putExtra("favorite_stalker_cmd",url);startActivity(hub);return;}try{android.content.Intent i=new android.content.Intent(android.content.Intent.ACTION_VIEW,android.net.Uri.parse(url));if(url.startsWith("content://"))i.addFlags(android.content.Intent.FLAG_GRANT_READ_URI_PERMISSION);startActivity(i);}catch(Exception e){Toast.makeText(FavoritesActivity.this,"Favorit kann nicht geöffnet werden",Toast.LENGTH_SHORT).show();}}
