...
public class AppsActivity extends Activity {
 ...
 final class AppView extends View{
  ...
  void load(){
   ...
   for(android.content.pm.ResolveInfo r:getPackageManager().queryIntentActivities(q,0)){String pkg=r.activityInfo.packageName;if(!pkg.equals(getPackageName()))apps.put(pkg,r.loadLabel(getPackageManager()).toString());}
   Intent tv=new Intent(Intent.ACTION_MAIN);tv.addCategory(Intent.CATEGORY_LEANBACK_LAUNCHER);
   for(android.content.pm.ResolveInfo r:getPackageManager().queryIntentActivities(tv,0)){String pkg=r.activityInfo.packageName;if(!pkg.equals(getPackageName()))apps.put(pkg,r.loadLabel(getPackageManager()).toString());}
   for(Map.Entry<String,String> e:apps.entrySet()){pkgs.add(e.getKey());names.add(e.getValue());}
  ...
  @Override public boolean onKeyDown(int key,KeyEvent e){if(key==KeyEvent.KEYCODE_DPAD_DOWN&&focus<names.size()-1)focus++;else if(key==KeyEvent.KEYCODE_DPAD_UP&&focus>0)focus--;else if((key==KeyEvent.KEYCODE_DPAD_CENTER||key==KeyEvent.KEYCODE_ENTER)&&!pkgs.isEmpty()){String pkg=pkgs.get(focus);Intent i=getPackageManager().getLeanbackLaunchIntentForPackage(pkg);if(i==null)i=getPackageManager().getLaunchIntentForPackage(pkg);if(i!=null)startActivity(i);else Toast.makeText(AppsActivity.this,"App kann nicht geöffnet werden",Toast.LENGTH_SHORT).show();}else if(key==KeyEvent.KEYCODE_BACK){finish();return true;}else return super.onKeyDown(key,e);invalidate();return true;}
 }
}
