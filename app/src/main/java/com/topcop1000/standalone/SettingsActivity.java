...
public class SettingsActivity extends Activity {
 ...
 final class SettingsView extends View{
  ...
  String[] rows(){return new String[]{"ZOMBIES: "+(prefs.getBoolean("zombies",true)?"EIN":"AUS"),"IDLE-MODUS: "+(prefs.getBoolean("idle",true)?"EIN":"AUS"),"EINSTELLUNGEN ZURÜCKSETZEN","ZURÜCK"};}
  ...
  void choose(){if(focus==0)prefs.edit().putBoolean("zombies",!prefs.getBoolean("zombies",true)).apply();else if(focus==1)prefs.edit().putBoolean("idle",!prefs.getBoolean("idle",true)).apply();else if(focus==2)prefs.edit().clear().apply();else finish();invalidate();}
  @Override public boolean onKeyDown(int k,KeyEvent e){if(k==KeyEvent.KEYCODE_DPAD_DOWN)focus=Math.min(3,focus+1);else if(k==KeyEvent.KEYCODE_DPAD_UP)focus=Math.max(0,focus-1);else if(k==KeyEvent.KEYCODE_DPAD_CENTER||k==KeyEvent.KEYCODE_ENTER)choose();else if(k==KeyEvent.KEYCODE_BACK){finish();return true;}else return super.onKeyDown(k,e);invalidate();return true;}
 }
}
